...
import ctcomp from "./ctcomp"
import PixImage, {ImageWithNavigate} from "./PixabayImage";
import "./apiget.css";
import Category from './CategoryComponent'
import TypeComponent from './TypeComponent'
import Video from './Video.mp4'

class Pixabay extends React.Component {
  baseUrl =
    "https://pixabay.com/api/?key=39584736-d653604d95086e2f2277cbc33&q=&image_type=photo&per_page=100";
  ...
  onClickX = (event) => {
    let searchatr = document.getElementById("sender-x").value.split(" ");

    

    let searchstr = "";
    for (let x = 0; x < searchatr.length; x++) {
      searchstr += searchatr[x] + "+";
    ...
    axios.get(this.baseUrl).then((res) => {
      ...
      res.data.hits.forEach(element => {
        console.log(element.imageWidth);
      });
    });
  };
  ...
  render() {
    return (
      <div>
        <div className="pix-search-div-gradient">
          <div>
            <svg
              viewBox="0 0 120 33"
        ...
        <div className="pix-search-div">
          <div className="xxx-video-xxx">
            <video className="bg-video-box" muted="" preload="auto" loop="true" autoplay=""> 
                < source src={Video} type="video/mp4"/>
            </video>
          </div>
        
            <div className="uwu-x">
                <h2>Amazing free images</h2>
                <p>Discover over 0.1 million photos and videos shared by our generous community.</p>
            </div>
          <div className="pix-container-hh">
            <BsSearch style={{fill: "black", width: 2 + "rem", alignSelf: "center"}}></BsSearch>
            <input
            ...
            ></input>
            <input
              className="pix-search-input-btn"
              type="button"
              value={"Search"}
        ...
        <div className="types-categories">
          <TypeComponent title={"House"} icon={"ri-home-line"} update={this.updateimagelist}></TypeComponent>
          <TypeComponent title={"Pics"} icon={"ri-camera-line"} update={this.updateimagelist}></TypeComponent>
          <TypeComponent title={"Illustrations"} icon={"ri-brush-line"} update={this.updateimagelist}></TypeComponent>
          <TypeComponent title={"Vectors"} icon={"ri-pen-nib-line"} update={this.updateimagelist}></TypeComponent>
          <TypeComponent title={"Videos"} icon={"ri-vidicon-fill"} update={this.updateimagelist}></TypeComponent>
          <TypeComponent title={"Music"} icon={"ri-music-line"} update={this.updateimagelist}></TypeComponent>
          <TypeComponent title={"Sound Effects"} icon={"ri-mist-line"} update={this.updateimagelist}></TypeComponent> 
          <TypeComponent title={"GIF files"} icon={"ri-fire-line"} update={this.updateimagelist}></TypeComponent>
        </div>

        <div className="categories">
           
           <Category title={"autumn"} update={this.updateimagelist}></Category>
           <Category title={"space"} update={this.updateimagelist}></Category>
           <Category title={"forest"} update={this.updateimagelist}></Category>
           <Category title={"city"} update={this.updateimagelist}></Category>
           <Category title={"flowers"} update={this.updateimagelist}></Category>
           <Category title={"water"} update={this.updateimagelist}></Category>
           <Category title={"sky"} update={this.updateimagelist}></Category>
           <Category title={"cat"} update={this.updateimagelist}></Category>
           <Category title={"night"} update={this.updateimagelist}></Category>

           <div className="drop-down-diw">
              <p>Selected <i class="ri-arrow-drop-down-line"></i></p>
           </div>
        </div>

        <div className="pix-custom-container">
          {this.state.images.map((el) => (
            <ImageWithNavigate url={el.webformatURL.replace("640","180")}> </ImageWithNavigate>
          ))}
        </div>
      </div>
      //    <input value={"get"} type="button" onClick={() =>{
